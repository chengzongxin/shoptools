...
class PriceReviewTab(ttk.Frame):
    ...
    def run_crawler(self, pages, cookie):
        """运行爬虫"""
        try:
            self.current_data = self.crawler.crawl(pages)
    ...
    def start_batch_processing(self):
        ...
        try:
            ...
            # 显示确认对话框
            if not messagebox.askyesno("确认", f"确定要批量处理 {pages} 页的待核价商品吗？"):
                return
    ...
    def run_batch_processing(self, pages):
        """运行批量处理"""
        try:
            results = self.crawler.batch_process_price_reviews(pages)
