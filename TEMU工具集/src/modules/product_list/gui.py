...
class ProductListGUI:
    ...
    def load_last_params(self):
        """加载上次保存的参数"""
        try:
            config_path = os.path.join('config', 'product_list_config.json')
    ...
    def save_params(self):
        ...
        try:
            ...
            with open(os.path.join('config', 'product_list_config.json'), 'w', encoding='utf-8') as f:
    ...
    def create_input_fields(self):
        # 页数输入
        ttk.Label(self.frame, text="获取页数:").grid(row=0, column=0, sticky=tk.W)
        self.pages_var = tk.StringVar(value=self.last_pages)
        self.pages_entry = ttk.Entry(self.frame, textvariable=self.pages_var, width=10)
        self.pages_entry.grid(row=0, column=1, sticky=tk.W)
        
        # 每页数据量输入
        ttk.Label(self.frame, text="每页数据量:").grid(row=1, column=0, sticky=tk.W)
        self.page_size_var = tk.StringVar(value=self.last_page_size)
        self.page_size_entry = ttk.Entry(self.frame, textvariable=self.page_size_var, width=10)
        self.page_size_entry.grid(row=1, column=1, sticky=tk.W)

    def create_cookie_input(self):
        # Cookie输入框架
        cookie_frame = ttk.LabelFrame(self.frame, text="请求头设置", padding="5")
    ...
    def create_log_area(self):
        # 创建日志文本框
        self.log_text = tk.Text(self.frame, height=15, width=80)
    ...
    def run_crawler(self, pages, page_size, cookie, anti_content, mallid):
        try:
            ...
            # 更新请求头
            crawler.set_headers(cookie, anti_content, mallid)
            
            # 获取数据
            logger.info(f"开始获取商品列表数据，计划获取 {pages} 页...")
